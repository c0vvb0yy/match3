...
func _ready():
	...
	GridManager.all_pieces = Util.make_2d_array(dimension.x, dimension.y)
	empty_spaces = Util.wrap_coordinates_around_grid(empty_spaces, dimension)
	GridManager.grid_dimension = dimension
	fill_grid()
	...
	for x in dimension.x:
		for y in dimension.y:
			if !Util.is_restricted_in_placement(Vector2(x,y), empty_spaces):
				...
				var loops = 0
				while(GridManager.is_match_at_short(x, y, piece) and loops < 100):
...
func _process(_delta):
	if(GridManager.grid_state != GridManager.GRID_STATES.wait):
		check_for_input()
...
func check_for_input():
	...
	if Util.is_in_grid(mouse_coord, dimension.x, dimension.y) and GridManager.is_piece_existing(mouse_coord.x, mouse_coord.y):
		if Input.is_action_just_pressed("mouse_click"):
			start_piece_pos = mouse_coord
			GridManager.all_pieces[start_piece_pos.x][start_piece_pos.y].multiply_scale(1.1)
...
func swap_pieces(coord:Vector2, dir:Vector2):
	...
	var selected_piece = GridManager.all_pieces[coord.x][coord.y]
	var target_piece = GridManager.all_pieces[target_coord.x][target_coord.y]
	if selected_piece != null and target_piece != null:
		GridManager.all_pieces[coord.x][coord.y] = target_piece
		GridManager.all_pieces[target_coord.x][target_coord.y] = selected_piece
		...
		match current_match[2]:
			direction.vertical:
				for i in range(y, y-amount, -1):
					GridManager.match_and_dim(GridManager.all_pieces[x][i])
			direction.horizontal:
				for i in range(x, x-amount, -1):
					GridManager.match_and_dim(GridManager.all_pieces[i][y])
	return GridManager.matches.size() > 0

func find_matches():
	var last_color = null
	var matched = 1
	GridManager.matches.clear()
	for x in dimension.x:
		for y in dimension.y:
			var current_color = null
			if(GridManager.is_piece_existing(x, y)):
				current_color = GridManager.all_pieces[x][y].color
		...
		if GridManager.is_piece_existing(x, dimension.y-1):
	...
	for y in dimension.y:
		for x in dimension.x:
			...
			if current_color == last_color and current_color != null:
				...
				matched = 1
			last_color = current_color
		if GridManager.is_piece_existing(dimension.x-1, y):
...
func store_match(x,y, amount:int, dir:direction, color):
	if amount < 3:
		return
	GridManager.matches.append([x,y, dir, amount, color])

func break_matches():
	for x in dimension.x:
		for y in dimension.y:
			if GridManager.is_piece_existing(x, y):
				if(GridManager.all_pieces[x][y].matched and !GridManager.is_match_at(dimension, x,y)):
					GridManager.unmatch(GridManager.all_pieces[x][y])
...
var score = 0 
func score_round():
	if(GridManager.matches.size() > 0 and GridManager.matches[GridManager.matches.size()-1] != null):
		for current_match in GridManager.matches:
		...
		for y in dimension.y:
			...
			and !Util.is_restricted_in_placement(Vector2(x,y), empty_spaces)):
				for i in range(y - 1, -1, -1):
					if GridManager.all_pieces[x][i] == null:
						break
					GridManager.all_pieces[x][i].fall(Util.grid_to_pixel(cell_size,Vector2(x,y)))
					GridManager.all_pieces[x][y] = GridManager.all_pieces[x][i]
					GridManager.all_pieces[x][i] = null
					make_current_pieces_fall()
					break
...
func end_round():
	...
	GridManager.disable_grid(false)
